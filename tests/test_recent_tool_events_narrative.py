def test_get_recent_tool_events_includes_narrative_and_transcript(monkeypatch):
    ...
    server.RECENT_TOOL_EVENTS.append(
        {
            ...
            "repo": "owner/repo",
            "ref": "main",
            "user_message": "Starting terminal_command (read) on owner/repo@main.",
        }
    )
    server.RECENT_TOOL_EVENTS.append(
        {
            ...
            "ref": "main",
            "duration_ms": 12,
            "user_message": "Finished terminal_command on owner/repo@main in 12ms.",
        }
    )
    ...
    assert any("Starting terminal_command" in msg for msg in res["narrative"])
    assert any("Finished terminal_command" in msg for msg in res["narrative"])
    assert "Starting terminal_command" in res["transcript"]
    assert "Finished terminal_command" in res["transcript"]
