...
@pytest.mark.anyio
async def test_terminal_command_auto_pushes_default_branch(monkeypatch, tmp_path):
    import github_mcp.workspace_tools.commands as commands
    ...
    async def run_shell(cmd: str, *, cwd: str, timeout_seconds: int, env=None):
        ...
        if cmd.startswith("git rev-parse --abbrev-ref --symbolic-full-name"):
    ...
    assert out["auto_push_branch"]["current_branch"] == "main"
    assert out["auto_push_branch"]["push"]["stdout"] == "pushed"
    assert any(cmd.startswith("git push -u origin main") for cmd in calls)
