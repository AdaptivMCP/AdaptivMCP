...
async def test_actions_endpoint_exposes_registered_tools():
    ...
    apply_action = next((a for a in data["actions"] if a.get("name") == "apply_text_update_and_commit"), None)
    assert apply_action is not None
    assert apply_action.get("title"), "expected actions endpoint to expose a title/display label"
    assert apply_action["title"] != apply_action["name"], "expected title to differ from raw tool name"
