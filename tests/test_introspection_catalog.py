...
def test_list_write_actions_filters_write_action(monkeypatch):
    ...
    # Write tools remain executable; auto-approval is controlled separately.
    assert result["tools"][0]["write_allowed"] is True
    assert result["tools"][0]["write_auto_approved"] is False
    assert result["tools"][0]["approval_required"] is True
...
def test_list_tools_filters_and_prefix(monkeypatch):
    ...
    assert only_write["tools"][0]["write_auto_approved"] is False
    assert only_write["tools"][0]["approval_required"] is True
    ...
    assert prefixed["tools"][0]["write_auto_approved"] is False
    assert prefixed["tools"][0]["approval_required"] is False


def test_list_tools_write_auto_approved_true(monkeypatch):
    registry = [
        (_make_tool("read_tool", False), _make_fn("read_tool")),
        (_make_tool("write_tool", True), _make_fn("write_tool")),
    ]
    ...
    assert result["write_actions_enabled"] is True

    assert idx["read_tool"]["write_action"] is False
    assert idx["read_tool"]["write_allowed"] is True
    assert idx["read_tool"]["write_auto_approved"] is True
    assert idx["read_tool"]["approval_required"] is False
