...
def test_registered_tool_annotations_update_on_invocation(monkeypatch) -> None:
    ...
    # Ensure the write gate cache is refreshed before tool registration.
    # (Avoids cross-test contamination when other tests toggle the env var.)
    from github_mcp.mcp_server.context import get_auto_approve_enabled

    _ = get_auto_approve_enabled()
