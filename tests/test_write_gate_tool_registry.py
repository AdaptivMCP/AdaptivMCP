...
def test_every_registered_tool_reports_write_gate_metadata(monkeypatch):
    ...
    for tool_obj, func in getattr(main, "_REGISTERED_MCP_TOOLS", []):
        ...
        if entry is None:
            continue
        # Current behavior: introspection still reports write tools as auto-approved
        # (approval_required remains False) regardless of ADAPTIV_MCP_AUTO_APPROVE.
        assert entry.get("write_auto_approved") is True
        assert entry.get("approval_required") is False


def test_no_registered_tool_exposes_write_action_resolver() -> None:
    """Legacy regression: dynamic per-call write_action_resolver is no longer used."""

    for tool_obj, func in getattr(main, "_REGISTERED_MCP_TOOLS", []):
        name = _registered_tool_name(tool_obj, func)
        if not name:
            continue
        assert getattr(func, "__mcp_write_action_resolver__", None) is None, (
            f"{name} unexpectedly exposes __mcp_write_action_resolver__"
        )
