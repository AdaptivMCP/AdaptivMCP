...
def test_load_body_from_sandbox_path(tmp_path):
    ...
    result = asyncio.run(_load_body_from_content_url(f"sandbox:{file_path}", context="test"))
...
def test_load_body_from_absolute_path(tmp_path):
    ...
    result = asyncio.run(_load_body_from_content_url(str(file_path), context="test"))
...
def test_load_body_from_invalid_scheme():
    with pytest.raises(GitHubAPIError):
        asyncio.run(_load_body_from_content_url("ftp://example.com/file", context="test"))
...
def test_load_body_from_sandbox_with_rewrite(monkeypatch):
    ...
    result = asyncio.run(_load_body_from_content_url("sandbox:/missing/file.txt", context="test"))
...
def test_load_body_from_absolute_with_rewrite(monkeypatch):
    ...
    result = asyncio.run(_load_body_from_content_url("/missing/abs.txt", context="test"))
...
def test_load_body_from_sandbox_missing_without_rewrite():
    with pytest.raises(GitHubAPIError) as excinfo:
        asyncio.run(_load_body_from_content_url("sandbox:/not/available.txt", context="missing"))
