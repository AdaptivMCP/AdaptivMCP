...
Highlights:
- Shared httpx.AsyncClient pools for GitHub and external HTTP requests.
- Fast raw file fetch helpers optimized for private repos.
- Read/write tool metadata preserved via a compatibility decorator so
  older FastMCP builds ignore unsupported arguments while clients can
  still introspect write_action intent.
"""
...
def _build_client(base_url: Optional[str] = None) -> httpx.AsyncClient:
    ...
    try:
        ...
        # Gracefully fall back if http2 dependencies are missing
        if "http2=True" in str(exc) or "h2" in str(exc):
            return httpx.AsyncClient(
                base_url=base_url,
                timeout=httpx.Timeout(HTTPX_TIMEOUT),
                limits=limits,
...
async def fetch_files(
    ...
    tasks = [asyncio.create_task(_fetch_one(p)) for p in paths]
