...
def controller_contract(compact: Optional[bool] = None) -> Dict[str, Any]:
    ...
    guardrails = [
        ...
        "After every commit that writes from the persistent workspace, re-clone the same branch with ensure_workspace_clone(reset=true) before running run_tests, repo-defined lint suites, or any other forward-moving actions (including additional edits, PR creation, or deployment-related checks) so verification always runs in a fresh workspace that matches GitHub state.\n",
