...
def controller_contract(compact: Optional[bool] = None) -> Dict[str, Any]:
    ...
    assistant_expectations = [
        ...
        "Prefer focused tools and diffs over large blobs. Use get_file_slice, get_file_with_line_numbers, and diff tools to work on specific files or ranges instead of sending huge inline payloads or command outputs.",
        ...
        "Treat failing tests, lint errors, and other breakages you introduce as your responsibility to fix: investigate them, say explicitly when you found or caused the issue, and update code, tests, and docs until the branch is green instead of leaving fixes to humans.\n",
        "When you discover bugs, misalignments, or inconsistencies in code, tests, or docs while working—even if they are not in the original request—fix them appropriately on the active feature branch and clearly mention these corrections in your summaries and PR descriptions.\n",
    ...
    controller_prompt_expectations = [
        ...
        "Keep safety, truncation, and large-file guidance visible so the controller prompt steers assistants toward slice-and-diff workflows instead of oversized responses or large payload retries.",
...
async def move_file(
    ...
    try:
        ...
        msg = str(exc)
        if "404" in msg:            delete_result = {"status": "noop", "reason": "source path missing"}
        else:
            raise
    else:
...
async def create_file(
    ...
    *,
    ...
    try:
        ...
        msg = str(exc)
        if "404" in msg:            sha_before: Optional[str] = None
        else:
            raise
    else:
...
async def apply_text_update_and_commit(
    ...
    *,
    ...
    try:
        ...
        msg = str(exc)
        if "404" in msg:            # The GitHub Contents API returns 404 when the file does not yet exist.
...
async def apply_patch_and_commit(
    ...
    *,
    ...
    try:
        ...
        msg = str(exc)
        if "404" in msg:            is_new_file = True
            old_text = ""
            sha_before = None
        else:
            raise
