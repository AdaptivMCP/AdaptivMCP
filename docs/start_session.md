...
- Use diff-oriented tools (`build_unified_diff`, `build_section_based_diff`, `apply_text_update_and_commit`, `apply_patch_and_commit`, `update_file_sections_and_commit`, `apply_line_edits_and_commit`) to propose and apply changes.
- Use `run_command` for small, focused commands (`rg`, `grep`, `sed -n 'start,endp'`, formatters, tests) and limit output size with flags such as `head` or explicit line ranges.
...
Controllers should keep the branch and workspace rules from `controller_contract` visible in their prompts. In short:
...
- Use `run_command` in that workspace for tests, linters, and small inspection commands.
