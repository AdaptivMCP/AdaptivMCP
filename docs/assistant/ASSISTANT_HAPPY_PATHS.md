...
This document is a **playbook for assistants** (such as Joeys GitHub) using the Adaptiv Controller GitHub MCP server. It provides concrete, repeatable **happy paths** that align with the controller prompts and workflows documented in this repository.

If you ever find yourself guessing or improvising a new flow, check this file and the rest of the docs together first. If there is no good happy path, *that* is a signal to add or improve one (in a docs branch, via PR) so that the docs stay aligned with the live server behavior.
...
6. When you need to understand branch state or CI health, call `get_latest_branch_status` for the feature branch you care about (and the base, typically `main`) instead of guessing from old workflow logs. Use that to decide whether a failure is tied to the current HEAD or an older commit.
