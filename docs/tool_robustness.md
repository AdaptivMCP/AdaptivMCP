...
The workspace tools operate on a persistent server-side clone (“repo mirror”). Most write flows should follow:
...
- All file path inputs are validated as repo-relative paths and must resolve within the repo root (path traversal is rejected).
...
- Prefer `delete_workspace_paths` over patch-based deletions in environments where unified diffs are blocked.
