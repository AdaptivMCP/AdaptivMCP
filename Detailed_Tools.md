...
Fetch the full UTF-8 contents of a single file at a ref. Prefer `open_file_context` or `get_file_with_line_numbers` for large files to avoid huge payloads.
...
Return a citation-friendly slice of a file with line numbers, wrapping an internal helper for large-file handling. Use this when summarizing or reviewing code.
