...
Write action: yes.
Write actions may be gated by server policy. If the server requires approval, the tool will return a structured error indicating approval is required.
...
Write action: no.
This tool is read-only and is not subject to write-approval gating.
...
- tool_name (string | null; optional)
...
- timeout_seconds (number; optional, default=300)
  ...
  Examples: 60, 300, 600
- use_temp_venv (boolean; optional, default=False)
...
Parameters:
- auto_setup_repo (unknown; optional, default=False)
...
- installing_dependencies (unknown; optional, default=False)
- lint_command (unknown; optional, default='ruff check .')
- preflight (unknown; optional, default=False)
- ref (unknown; optional, default='main')
  Git ref to operate on. Typically a branch name, but may also be a tag or commit SHA. Defaults to 'main' when available.
  Examples: 'main', 'develop', 'feature/my-branch'
