...
async def comment_on_pull_request(
    full_name: str, number: int, body: str
) -> dict[str, Any]:
...
async def _build_default_pr_body(
    *,
    ...
    lines.append(
        f"- Status: {'Draft (still in progress)' if draft else 'Ready for review'}"
    )
    lines.append("")
    ...
    lines.append(
        "<!-- Default PR body generated by chatgpt-mcp-github. Edit freely. -->"
    )
...
async def open_pr_for_existing_branch(
    ...
    is_no_commits_conflict = bool(
        status_code == 422 and "no commits between" in conflict_hint
    )
...
async def get_pr_overview(full_name: str, pull_number: int) -> dict[str, Any]:
    ...
    files: list[dict[str, Any]] = []
    try:
        files_resp = await m.list_pr_changed_filenames(
            full_name, pull_number, per_page=100
        )
